...
func BenchmarkWaitersAbandon(b *testing.B) {
	...
	sel := func(<-chan struct{}) bool { return false }
	b.ReportAllocs()
	for b.Loop() {
...
func BenchmarkWaitersNotified(b *testing.B) {
	...
	sel := func(ch <-chan struct{}) bool { <-ch; return true }
	b.ReportAllocs()
	for b.Loop() {
		done := make(chan struct{})
		go func() { w.WaitFunc(confirm, sel); close(done) }()
		for !w.wakeOne() { // pop the registered waiter
		}
		<-done
	}
}
...
func TestWaitersReapPreservesLiveWaiter(t *testing.T) {
	...
	go func() {
		w.WaitFunc(
			...
			func(ch <-chan struct{}) bool { <-ch; close(gotA); return true },
		)
	}()
	...
	for range reapBudget + 2 {
		w.WaitFunc(
			...
			func(<-chan struct{}) bool { return false },
		)
	}
