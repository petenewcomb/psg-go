...
An important edge case occurs when only one sample is available: since linearity testing requires at least two samples plus the origin point, a single sample provides insufficient data to determine whether throughput scaling is linear or non-linear. The algorithm handles this by treating single-sample scenarios as insufficient data rather than evidence of non-linear behavior.
