...
//
// 1. Task Functions: The context passed to TaskFunc should be respected
//    for cancellation. User code should propagate this context to any operations
//    performed within the task, rather than creating new, disconnected contexts.
//    While the job will still function if a task ignores cancellation, such tasks
//    may continue running until they complete naturally, even after the job has
//    been canceled.
//
// 2. Gather Functions: The context passed to GatherFunc is the context from the
//    calling Scatter or Gather* method, not the job's context. User code should
//    propagate this context, especially when calling Scatter to create new tasks.
//
...
//
// 1. All running TaskFuncs receive context cancellation but will run until they
//    return. The library will correctly clean up once they complete.
//
// 2. Combiners will be flushed to ensure no data is lost.
//
// 3. Job.CancelAndWait guarantees that all task goroutines exit before returning.
//
...
//
// 2. Gather Queuing: Gather operations are queued rather than processed recursively
//    to prevent stack overflow when gather functions launch new tasks.
//
...
// # Best Practices
//
// 1. Always use the provided context in TaskFunc, GatherFunc, and Combiner methods.
//
// 2. Follow the pattern of checking ctx.Done() regularly in long-running tasks.
//
// 3. Don't create new root contexts (e.g., context.Background()) within functions
//    that receive a context from the library.
//
// 4. Launch new tasks from GatherFunc or Combiner methods, not from TaskFunc.
//
// 5. For tasks that need to create internal concurrency, consider creating a sub-job
//    within the task rather than calling Scatter directly.
package psg
