# Development history — combiner branch refactor journal

> **Historical record, excised from `WORKING_NOTES.md` on 2026-06-23.** This is the
> dated implementation/design journal of the combiner-branch reshape: the
> dispatch/execution architecture pivot, the `psg.Pool`→`Wave` fold, the rdvq/workq
> consolidation, limiter suspend/resume, and earlier hang root-causes. It is kept for
> archaeology and rationale — it is **not** current status.
>
> - Current branch status + live banners: `WORKING_NOTES.md`.
> - Current/target design + superseded-surface rationale: the other `docs/decisions/`
>   docs — `surface-lineage.md`, `body-context-pool.md`, `meta-context-migration.md`,
>   `permit-core.md`, `backpressure-and-reentrancy.md`, etc.
>
> Entries accreted roughly reverse-chronologically; dates are inline. Relative
> cross-references ("the ⚠ HANG note below", "see … above") are internal to THIS file.
