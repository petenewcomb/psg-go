...
type Skimmer[T any] struct {
	...
	skimFn streampool.HandlerFunc[TaskResult[T]]
}

func NewSkimmer[T any](c *Controller, skimFn streampool.HandlerFunc[T]) {
	// TODO: pool
	g := &Skimmer[T]{
		controller: c,
		wrappedFn:  skimFn,
...
func (g *Skimmer[T]) SkimFn() streampool.HandlerFunc[TaskResult[T]] {
	if g.skimFn == nil {
		g.skimFn = g.skim
	}
	return g.skimFn
