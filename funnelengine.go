...
type funnelEngine struct {
	...
	// funnels registers the funnels created on this wave (heterogeneous T via the
	// instanceRecycler interface) so the end-of-work flush sweep can return their
	// spent instance shells to the pool. Registration is during the wave's Open phase;
	// recycling at Flushing (Open < Flushing); a mutex guards concurrent NewFunnel.
	// This is the funnels' ONLY tie to the engine — they hold no reference and have no
	// other teardown.
	funnelsMu sync.Mutex
	funnels   []instanceRecycler
...
// sweep can recycle its spent instance shells. Called from NewFunnel during Open.
func (fe *funnelEngine) registerFunnel(r instanceRecycler) {
	fe.funnelsMu.Lock()
	fe.funnels = append(fe.funnels, r)
	fe.funnelsMu.Unlock()
}

// recycleFunnelInstances returns every registered funnel's spent instance shells to
// its pool. Called once from the end-of-work flush sweep, AFTER all instances have
// been force-flushed. Idempotent: it takes the slice and leaves the registry empty.
func (fe *funnelEngine) recycleFunnelInstances() {
	fe.funnelsMu.Lock()
	funnels := fe.funnels
	fe.funnels = nil
	fe.funnelsMu.Unlock()
	for _, r := range funnels {
		r.recycleSpentInstances()
	}
}
