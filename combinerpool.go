...
func getCombineFunc[I any, O any](cm *combinerMap, j *Job, c *Combine[I, O]) halfBoundCombineFunc[I] {
	...
	if bc != nil {
		...
		combineFunc = func(ctx context.Context, input I, inputErr error) {
			...
			// The job's in-flight task counter must be decremented here just as
			// it is in Job.executeGather.
...
func NewCombinerPool(ctx context.Context, concurrencyLimit int) *CombinerPool {
	if ctx == nil {
...
func (cp *CombinerPool) launch(ctx context.Context, j *Job, combine boundCombineFunc) {

	// Loop in case the concurrency limit changes
	for {
		concurrencyLimit, concurrencyLimitChangeCh := cp.concurrencyLimit.Load()

		if cp.inFlight.Increment() && cp.liveCount.Load() == 0 {
...
func (cp *CombinerPool) launchNewCombiner(j *Job, concurrencyLimit int, combine boundCombineFunc) bool {
	if cp.liveCount.Add(1) > int64(concurrencyLimit) {
		cp.liveCount.Add(-1)
		return false
	}
	...
	go func() {
		...
		goroutineCtx = withBackpressureProvider(cp.ctx, bp)
