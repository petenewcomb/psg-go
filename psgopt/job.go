...
// WithGCBackpressureSettings sets both the GC time ratio threshold and monitoring frequency for [github.com/petenewcomb/psg-go.Job].
// This is a convenience function for configuring GC-based backpressure settings at once.
//
// For setting individual GC parameters, see [WithMaxGCTimeRatioThreshold] and [WithGCTimeUpdateInterval].
// The default values are [DefaultMaxGCTimeRatioThreshold] and [DefaultGCTimeUpdateInterval].
//
// Setting threshold to 0 disables GC-based backpressure entirely.
// Setting interval to 0 disables GC monitoring entirely, which also disables GC-based backpressure.
...
// WithMaxGCTimeRatioThreshold sets the threshold for GC time ratio that triggers
// backpressure during [github.com/petenewcomb/psg-go.Job] scatter operations. When the ratio of GC CPU time to total
...
// Related: [WithGCTimeUpdateInterval] controls monitoring frequency, and [WithGCBackpressureSettings] sets both at once.
...
// WithGCTimeUpdateInterval sets how frequently [github.com/petenewcomb/psg-go.Job] monitors GC time ratios
...
// Related: [WithMaxGCTimeRatioThreshold] sets the threshold that triggers backpressure, and [WithGCBackpressureSettings] sets both at once.
...
// (Job.Gather, Job.TryGather, Job.GatherAll, Job.TryGatherAll, Job.CloseAndGatherAll),
// Gather.Scatter, or Job.Close if no tasks are in flight at the time of closing.
//
