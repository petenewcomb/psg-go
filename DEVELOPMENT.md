...
- Do not commit changes that touch concurrency-sensitive code until a large batch of `TestBySimulation` runs is green under `-race`; small samples miss rare interleavings.  Set `-timeout` generously (a `-race` batch at high check counts takes minutes), as a too-tight timeout manifests as a false hang.
  - Exception — checkpoint commits on a development branch when the gate cannot bind: mid-refactor, the full-configuration gate may fail for reasons the change did not introduce (a known pre-existing hang family) or may be unable to pass until a later phase of the same agreed plan lands.  A checkpoint commit is then permitted, provided all three of the following hold: (1) baseline parity is demonstrated, not assumed — run the same configuration on the unmodified baseline (a worktree control) and confirm it fails the same way, and investigate before concluding when the failure mode differs; (2) every gate that can bind is green — package suites under `-race`, and `TestBySimulation` `-race` batches at a configuration both trees pass (e.g. `-short`); (3) the unbound obligation is recorded in `WORKING_NOTES.md` with the evidence, so it is re-armed deliberately rather than dropped — the full gate still binds before the phase that makes it satisfiable is called done, and always before merge.
